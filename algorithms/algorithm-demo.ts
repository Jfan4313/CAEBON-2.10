...
// 计算基准
const lightingBaseline = calculateLightingBaseline(lightingParams);
...
// 计算节能潜力
const lightingSavings = calculateLightingSavings(lightingParams, oldLightingSystem);
console.log('\n2. 照明系统节能潜力:');
...
const environmentalFactors = {
  region: '上海',
  avgTemperature: 26,
  annualSunshineHours: getCitySunshineData('上海'),
  peakPrice: 1.2,
  valleyPrice: 0.35,
  flatPrice: 0.6,
  cloudCover: 30,
...
// 计算节能潜力
const hvacSavings = calculateHVACSavings(hvacParams, oldHVACSystem, environmentalFactors, trafficPattern);
console.log('\n4. 暖通空调系统节能潜力:');
...
// 计算储能优化
const priceCurve = generateDefaultPriceCurve(1.2, 0.35, 0.6);
...
const integratedResult = calculateIntegratedOptimization(
  {
    area: 1000,
    totalPower: 20,
    dailyHours: 8,
...
const comfortMetrics = calculateComfortMetrics(
  24,      // 温度
  60,      // 湿度
  0.1,      // 空气流速
  24,      // 平均辐射温度
...
[20, 22, 24, 26, 28].forEach(temp => {
  const metrics = calculateComfortMetrics(temp, 50, 0.1, 24, 0.5, 1.0);
...
const batteryHealth = calculateBatteryDegradation(
  'LFP',      // 磷酸铁锂电池
  2000,      // 使用3年
  90,        // 90%放电深度
  25,         // 25°C运行温度
...
const v2gSchedules = calculateV2GOptimization(v2gDevices, priceCurve);
...
for (let hour = 0; hour < 24; hour++) {
  const thermalState = calculateDigitalTwinThermalModel(
    buildingParams,
    environmentalFactors,
    digitalTwinModel,
    undefined,
...
const rlTrainingResult = runRLOptimization(
  {
...
console.log('\n提示: 在algorithms目录下运行: ts-node algorithm-demo.js');
