...
export default function RetrofitCarbon() {
  ...
  const aggregatedData = useMemo(() => {
      // 1. Calculate Electricity Savings (kWh)
      let totalSavingMoney = 0;
      (Object.values(modules) as ModuleData[]).forEach(m => {
          if (m.isActive && m.id !== 'retrofit-solar' && m.id !== 'retrofit-carbon') {
              totalSavingMoney += m.yearlySaving;
          }
      });
      const avgPrice = priceConfig.mode === 'fixed' ? priceConfig.fixedPrice : 0.85; 
      const savingKWh = (totalSavingMoney * 10000) / avgPrice;

      // 2. Calculate Green Power Generation (kWh)
      const solarModule = modules['retrofit-solar'];
      let solarKWh = 0;
      if (solarModule?.isActive) {
          const capMatch = solarModule.kpiPrimary.value.match(/(\d+(\.\d+)?)/);
          const capacity = capMatch ? parseFloat(capMatch[0]) : 0;
          solarKWh = capacity * 1100; 
      }

      return {
          savingKWh, // Contribution to Energy Efficiency
          solarKWh,  // Contribution to Green Energy Replacement
          totalElecOptimized: savingKWh + solarKWh
      };
  }, [modules, priceConfig]);
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="flex gap-4 items-start">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="text-xs text-slate-500 mb-2">外购电力产生的隐含碳排放 (自动关联改造模块)</p>
                                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex gap-6">
                                                <div>
                                                    <div className="text-[10px] text-slate-400">节能改造减排</div>
                                                    <div className="text-sm font-bold text-emerald-600">{carbonPhysics.reductionFromSaving.toFixed(1)} t</div>
                                                </div>
                                                <div>
                                                    <div className="text-[10px] text-slate-400">光伏替代减排</div>
                                                    <div className="text-sm font-bold text-amber-500">{carbonPhysics.reductionFromSolar.toFixed(1)} t</div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
