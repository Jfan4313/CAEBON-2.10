...
export default function RetrofitLighting() {
    ...
    const financials = useMemo(() => {
        ...
        // Calculate electricity price based on priceConfig mode
        let electricityPrice = 0.85;
        if (priceConfig.mode === 'fixed') {
            electricityPrice = priceConfig.fixedPrice;
            ...
            electricityPrice = totalDuration > 0 ? weightedSum / totalDuration : 0.85;
            ...
            electricityPrice = avgSpotPrice || 0.85;
        }
        ...
        const ownerBenefit = isEmc ? annualSavingMoney * ownerShareRate : annualSavingMoney;
        const investorRevenue = isEmc ? annualSavingMoney * (1 - ownerShareRate) : annualSavingMoney;

        // Investment firm's perspective cashflows
        const cashFlows = [-investment];
        for (let i = 0; i < 5; i++) cashFlows.push(investorRevenue);
    ...
    }, [mode, quickBuildings, preciseState, priceConfig, investmentConfig]);
    ...
    // Sync to Context
    useEffect(() => {
        const newParams = { mode, quickBuildings, preciseState, investmentConfig };
        ...
        if (currentStoredParams !== newParamsString) {
            updateModule('retrofit-lighting', {
                ...
                kpiSecondary: { label: '节电', value: `${(financials.energySaving / 10000).toFixed(1)} 万度` },
                params: newParams
            });
        }
    }, [mode, quickBuildings, preciseState, investmentConfig, financials, updateModule, currentModule.params]);
                                    ...
                                    <div className="p-5 bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl border border-orange-200">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div className="space-y-1">
                                                ...
                                                />
                                                <p className="text-[10px] text-orange-400">业主获得节能效益的 {investmentConfig.emcOwnerShareRate}%</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
