...
const data = [
  { name: '1月', val: 98.5 }, { name: '2月', val: 99.0 }, { name: '3月', val: 99.2 },
  { name: '4月', val: 99.5 }, { name: '5月', val: 99.8 }, { name: '6月', val: 99.9 },
  { name: '7月', val: 99.9 }, { name: '8月', val: 99.9 }, { name: '9月', val: 99.5 },
  { name: '10月', val: 99.2 }, { name: '11月', val: 99.0 }, { name: '12月', val: 98.8 },
];
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className={`flex-1 overflow-y-auto p-8 pb-32 transition-opacity duration-300 ${currentModule.isActive ? 'opacity-100' : 'opacity-50 pointer-events-none grayscale'}`}>
            <div className="max-w-5xl mx-auto space-y-6">
                ...
                {/* Features */}
                <section className="bg-gradient-to-br from-indigo-50 to-white rounded-xl shadow-sm border border-indigo-100 p-6">
                    <h3 className="text-base font-bold text-indigo-800 mb-4 flex items-center gap-2">
                        <span className="material-icons">grid_view</span> 核心功能介绍
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            {title: '源网荷储协同', desc: '统一调度光伏、储能与负荷，最大化清洁能源自用率，降低用电成本。'},
                            {title: '孤岛运行能力', desc: '具备PCC点快速离网控制，在电网故障时保障关键负荷不间断供电。'},
                            {title: '电能质量优化', desc: '通过储能变流器主动支撑电压与频率，提升园区供电稳定性。'}
                        ].map((f, i) => (
                ...
                <div className="flex-1 w-full min-h-0 bg-slate-50 rounded-xl border border-slate-100 p-4">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={data} margin={{top: 20, right: 30, left: 20, bottom: 5}} barSize={40}>
                            ...
                            <YAxis tick={{fontSize: 12, fill: '#94a3b8'}} axisLine={false} tickLine={false} domain={[98, 100]} />
                            <Tooltip contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)'}} />
                            <Bar dataKey="val" name="可靠性(%)" fill="#6366f1" radius={[6,6,0,0]} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
