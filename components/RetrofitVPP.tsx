...
export default function RetrofitVPP() {
    ...
    const aggregation = useMemo(() => {
        ...
        // Extract raw capacities with safe fallbacks and cross-module linkages
        const microgridModule = modules['retrofit-microgrid'];

        // Storage capacity: if Storage module on, use its power; else if Microgrid has storage, use guess; else 0
        let stCap = 0;
        if (storageModule?.isActive) {
            stCap = getVal(storageModule, ['basicParams', 'power']);
            if (!stCap && storageModule.kpiPrimary?.value) {
                const match = storageModule.kpiPrimary.value.match(/(\d+(\.\d+)?)/);
                if (match) stCap = parseFloat(match[0]);
            }
        } else if (microgridModule?.isActive && microgridModule.params?.quickState?.scope?.storage) {
            stCap = 100; // rough guess if governed by MG
        }

        // EV capacity
        let evCap = 0;
        if (evModule?.isActive) {
            const dcCount = getVal(evModule, ['fleet', 'dcChargers']) || 2;
            const acCount = getVal(evModule, ['fleet', 'acChargers']) || 10;
            evCap = (dcCount * 120) + (acCount * 7);
        } else if (microgridModule?.isActive && microgridModule.params?.quickState?.scope?.ev) {
            evCap = 120 * 2;
        }

        // HVAC capacity
        let hvacCap = 0;
        if (hvacModule?.isActive) {
            const blds = hvacModule.params?.hvacBuildings || [];
            hvacCap = blds.reduce((sum: number, b: any) => sum + (b.active ? b.load : 0), 0);
        }

        // Lighting capacity
        let lightingCap = 0;
        if (lightingModule?.isActive) {
            const blds = lightingModule.params?.buildings || [];
            lightingCap = blds.reduce((sum: number, b: any) => sum + (b.active ? b.area * 15 / 1000 : 0), 0); // approx 15W/sqm
        }

        const raw = {
            storage: stCap || 100, // keep minimal fallback for UI demo if nothing is on
            ev: evCap || 310,
            hvac: hvacCap || 1650,
            lighting: lightingCap || 105
        };

        // Determine adjustable factors based on smart control presence
        // If Microgrid AI is active, response reliability/depth increases
        const hasSmartControl = microgridModule?.isActive;
        const storageFactor = hasSmartControl ? 0.95 : RESOURCE_FACTORS.storage.factor;
        const hvacFactor = hasSmartControl ? 0.25 : RESOURCE_FACTORS.hvac.factor;

        // Calculate Adjustables
        const adjustables = {
            storage: raw.storage * storageFactor,
            ev: raw.ev * RESOURCE_FACTORS.ev.factor,
            hvac: raw.hvac * hvacFactor,
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                ...
                <div className={`flex-1 overflow-y-auto p-6 pb-32 transition-opacity duration-300 ${currentModule.isActive ? 'opacity-100' : 'opacity-50 pointer-events-none grayscale'}`}>
                    <div className="max-w-7xl mx-auto space-y-4">
                        ...
                        {/* NEW: 5-Year Revenue Roadmap */}
                        <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-sm font-bold text-slate-800 flex items-center gap-2">
                                    <span className="material-icons text-indigo-500 text-lg">insights</span>
                                    VPP 五年收益增长路径预估
                                </h3>
                                <div className="text-[10px] text-slate-400 bg-slate-50 px-2 py-1 rounded">随市场政策完善，增长率逐年提升</div>
                            </div>
                            <div className="h-52">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart
                                        data={[
                                            { year: '第1年', cap: parseFloat((financials.netRevenue * 0.4).toFixed(1)), energy: parseFloat((financials.netRevenue * 0.4).toFixed(1)), spot: parseFloat((financials.netRevenue * 0.2).toFixed(1)) },
                                            { year: '第2年', cap: parseFloat((financials.netRevenue * 0.38).toFixed(1)), energy: parseFloat((financials.netRevenue * 0.45).toFixed(1)), spot: parseFloat((financials.netRevenue * 0.3).toFixed(1)) },
                                            { year: '第3年', cap: parseFloat((financials.netRevenue * 0.35).toFixed(1)), energy: parseFloat((financials.netRevenue * 0.5).toFixed(1)), spot: parseFloat((financials.netRevenue * 0.5).toFixed(1)) },
                                            { year: '第4年', cap: parseFloat((financials.netRevenue * 0.3).toFixed(1)), energy: parseFloat((financials.netRevenue * 0.55).toFixed(1)), spot: parseFloat((financials.netRevenue * 0.75).toFixed(1)) },
                                            { year: '第5年', cap: parseFloat((financials.netRevenue * 0.28).toFixed(1)), energy: parseFloat((financials.netRevenue * 0.6).toFixed(1)), spot: parseFloat((financials.netRevenue * 1.0).toFixed(1)) },
                                        ]}
                                    ...
                                    >
                                        ...
                                        <Bar dataKey="cap" name="容量补贴" stackId="a" fill="#818cf8" />
                                        <Bar dataKey="energy" name="电量响应" stackId="a" fill="#34d399" />
                                        <Bar dataKey="spot" name="现货套利" stackId="a" fill="#fbbf24" radius={[4, 4, 0, 0]} />
                                    </BarChart>
                                </ResponsiveContainer>
                            </div>
                            <div className="mt-3 text-[10px] text-slate-400 text-center">
                                * 现货套利收益随电力现货市场成熟度逐步释放，第5年预计达到当前基础补贴的 <span className="font-bold text-slate-600">{(financials.netRevenue > 0 ? ((financials.netRevenue * 1.88 / financials.netRevenue - 1) * 100).toFixed(0) : 0)}%</span> 增幅
                            </div>
                        </section>
                    </div>
                </div>
