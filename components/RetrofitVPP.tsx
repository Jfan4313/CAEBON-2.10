...
const RetrofitVPP: React.FC = () => {
  const [strategy, setStrategy] = useState<'dr' | 'as'>('dr');
            ...
            <div className="max-w-4xl mx-auto space-y-6">
                ...
                <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label className={`cursor-pointer group relative p-4 rounded-lg border-2 transition-all h-full shadow-sm flex flex-col ${strategy === 'dr' ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-slate-200 hover:border-primary/50'}`}>
                            <input type="radio" name="strategy" className="sr-only" checked={strategy === 'dr'} onChange={() => setStrategy('dr')} />
                            <div className="flex justify-between items-start mb-2">
                                <div className="w-10 h-10 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center">
                                    <span className="material-icons">notifications_active</span>
                                </div>
                                {strategy === 'dr' && <span className="material-icons text-primary">check_circle</span>}
                            </div>
                            <h4 className="font-bold text-slate-900 mb-1">削峰填谷需求响应 (DR)</h4>
                            <p className="text-xs text-slate-500 leading-relaxed">响应电网邀约，在用电高峰期通过调节空调、储能等柔性负荷削减功率，获取补贴。</p>
                        </label>

                        <label className={`cursor-pointer group relative p-4 rounded-lg border-2 transition-all h-full shadow-sm flex flex-col ${strategy === 'as' ? 'border-primary bg-primary/5 ring-1 ring-primary/20' : 'border-slate-200 hover:border-primary/50'}`}>
                            <input type="radio" name="strategy" className="sr-only" checked={strategy === 'as'} onChange={() => setStrategy('as')} />
                            <div className="flex justify-between items-start mb-2">
                                <div className="w-10 h-10 rounded-full bg-violet-100 text-violet-600 flex items-center justify-center">
                                    <span className="material-icons">speed</span>
                                </div>
                                {strategy === 'as' && <span className="material-icons text-primary">check_circle</span>}
                            </div>
                            <h4 className="font-bold text-slate-900 mb-1">电力辅助服务市场</h4>
                            <p className="text-xs text-slate-500 leading-relaxed">参与调频、备用等实时电力辅助服务市场，对设备的响应速度和控制精度要求更高。</p>
                        </label>
                    </div>
                </section>
                ...
                <section className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1.5">响应时间要求</label>
                            <div className="relative">
                                <select className="block w-full rounded-lg border-slate-300 bg-white shadow-sm focus:border-primary focus:ring-primary sm:text-sm py-2.5 px-3 border outline-none">
                                    <option>分钟级 (DR)</option>
                                    <option>秒级 (调频)</option>
                                </select>
                            </div>
                        </div>
            ...
            >
                <div className="flex justify-between items-center mb-6">
                    <div>
                        ...
                        <p className="text-slate-500 mt-1 ml-12">基于电网调度需求与响应能力的模拟结果</p>
                    </div>
                    <button 
                        onClick={() => setIsChartExpanded(false)}
