                                    ...
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart
                                            ...
                                            data={[
                                                {
                                                    name: '建设成本 (CAPEX) ±10%',
                                                    low: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment * 1.1 : analysisData.firstYearSaving - (analysisData.totalInvestment * 1.1 * ((projectBaseInfo.omRate ?? params.opexRate) / 100)))) - simulation.irr).toFixed(2)),
                                                    high: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment * 0.9 : analysisData.firstYearSaving - (analysisData.totalInvestment * 0.9 * ((projectBaseInfo.omRate ?? params.opexRate) / 100)))) - simulation.irr).toFixed(2))
                                                },
                                                {
                                                    name: '基础电价 (Tariff) ±10%',
                                                    low: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment : (analysisData.firstYearSaving * 0.9) - (analysisData.totalInvestment * ((projectBaseInfo.omRate ?? params.opexRate) / 100)))) - simulation.irr).toFixed(2)),
                                                    high: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment : (analysisData.firstYearSaving * 1.1) - (analysisData.totalInvestment * ((projectBaseInfo.omRate ?? params.opexRate) / 100)))) - simulation.irr).toFixed(2))
                                                },
                                                {
                                                    name: '系统运维 (O&M) ±20%',
                                                    low: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment : analysisData.firstYearSaving - (analysisData.totalInvestment * ((projectBaseInfo.omRate ?? params.opexRate) * 1.2 / 100)))) - simulation.irr).toFixed(2)),
                                                    high: parseFloat((calculateIRR(Array.from({ length: params.period + 1 }, (_, i) => i === 0 ? -analysisData.totalInvestment : analysisData.firstYearSaving - (analysisData.totalInvestment * ((projectBaseInfo.omRate ?? params.opexRate) * 0.8 / 100)))) - simulation.irr).toFixed(2))
                                                }
                                            ]}
